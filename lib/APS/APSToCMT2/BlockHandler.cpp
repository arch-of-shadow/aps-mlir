...
LogicalResult BlockHandler::processFunctionAsBlocks() {
  ...
  if (failed(identifyBlocksByFuncOp()))
    return funcOp.emitError()
           << "APSToCMT2 block handling failed while identifying function "
              "blocks";
  ...
  for (BlockInfo &block : blocks) {
    if (block.type == BlockType::REGULAR) {
      block.captures_rocc_command = true;
      break;
    }
  }
  ...
  if (failed(processAllBlocks()))
    return funcOp.emitError()
           << "APSToCMT2 block handling failed while processing generated "
              "blocks";
...
LogicalResult BlockHandler::identifyBlocksByLoop(tor::ForOp loopOp) {
  ...
  if (failed(segmentBlockIntoBlocks(loopBody, blockId))) {
    return loopOp.emitError()
           << "APSToCMT2 loop block handling failed while segmenting loop "
              "body";
  }
...
LogicalResult BlockHandler::processLoopBodyAsBlocks(tor::ForOp loopOp) {
  ...
  if (failed(identifyBlocksByLoop(loopOp)))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while identifying body "
              "blocks";
  ...
  if (failed(analyzeCrossBlockDataflow()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while analyzing "
              "cross-block dataflow";

  // Phase 2: block-to-block data infrastructure.
  if (failed(createCrossBlockValueRegs()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while creating cross-block "
              "value storage";

  // Create token FIFOs between sequential sub-blocks.
  if (failed(createBlockTokenFIFOs()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while creating block token "
              "FIFOs";

  if (pipelineMode && failed(createPipelineInputFanoutFIFOs()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while creating pipeline "
              "input fanout FIFOs";

  // Phase 2.5: parent live-in capture for multi-consumer sub-block inputs.
  if (failed(analyzeInputDistributionNeeds()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while analyzing input "
              "distribution";

  if (failed(createInputDistributionRegs()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while creating input "
              "distribution registers";

  if (failed(generateInputCaptureRule()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while generating input "
              "capture rule";
  ...
  if (failed(processAllBlocks()))
    return loopOp.emitError()
           << "APSToCMT2 loop body handling failed while processing generated "
              "body blocks";

  return success();
}

LogicalResult BlockHandler::processRegionAsBlocks(Block *regionBlock,
                                                  Operation *anchorOp) {
  llvm::dbgs() << "[BlockHandler] Processing nested region as unified blocks\n";

  if (!regionBlock)
    return success();

  unsigned blockId = 0;
  if (failed(segmentBlockIntoBlocks(regionBlock, blockId)))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while segmenting body";

  if (blocks.empty()) {
    llvm::dbgs() << "[BlockHandler] No blocks found in nested region\n";
    return success();
  }

  if (failed(analyzeCrossBlockDataflow()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while analyzing "
              "cross-block dataflow";

  if (failed(createCrossBlockValueRegs()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while creating "
              "cross-block value storage";

  if (failed(createBlockTokenFIFOs()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while creating block "
              "token FIFOs";

  if (pipelineMode && failed(createPipelineInputFanoutFIFOs()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while creating pipeline "
              "input fanout FIFOs";

  if (failed(analyzeInputDistributionNeeds()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while analyzing input "
              "distribution";

  if (failed(createInputDistributionRegs()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while creating input "
              "distribution registers";

  if (failed(generateInputCaptureRule()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while generating input "
              "capture rule";

  if (failed(processAllBlocks()))
    return anchorOp->emitError()
           << "APSToCMT2 region block handling failed while processing nested "
              "blocks";
...
LogicalResult BlockHandler::segmentBlockIntoBlocks(Block *mlirBlock, unsigned &blockId) {
  ...
  for (size_t segIdx = 0; segIdx < blockSegments.size(); ++segIdx) {
    ...
    // Do not copy parent output FIFOs to every segment here.  Parent outputs
    // are assigned later in processAllBlocks() only to the last sub-block (or
    // to explicit pipeline live edges).  Copying them here lets an earlier
    // nested loop enqueue a parent yield/result FIFO before the real producer
    // block runs, which can fill the FIFO and deadlock the loop controller.
    ...
    if (type == BlockType::CONDITIONAL_THEN) {
      block.is_conditional_block = true;
    }
...
LogicalResult BlockHandler::processBlock(BlockInfo& block) {
  ...
  if (block.is_conditional_block || block.type == BlockType::CONDITIONAL_THEN ||
    ...
    std::string ifPrefix = block.blockName + "_";
    IfHandler ifHandler(pass, mainModule, funcOp, poolInstance, roccInstance,
                        hellaMemInstance, dmaItfc, csrItfc, circuit, mainClk,
                        mainRst, instructionId, regRdInstance,
                        block.input_token_fifo, block.output_token_fifo,
                        block.input_fifos, block.output_fifos, ifPrefix);
    ifHandler.setRequireContextToken(pipelineMode);
    return ifHandler.processIfBlock(block);
  }
...
LogicalResult BlockHandler::createCrossBlockValueRegs() {
  ...
  llvm::DenseMap<std::tuple<Value, unsigned, unsigned>, Instance *>
      pipelineEdgeStorage;
  auto &builder = mainModule->getBuilder();
  auto savedIP = builder.saveInsertionPoint();
  ...
  for (CrossBlockValueFlow &flow : crossBlockFlows) {
    ...
    if (pipelineMode) {
      if (producerBlock->blockId >= consumerBlock->blockId) {
        return consumerBlock->mlirBlock->getParentOp()->emitError()
               << "APSToCMT2 pipeline dataflow requires producer block "
                  "before consumer block for value live-through";
      }

      Instance *lastFIFO = nullptr;
      for (unsigned edgeFrom = producerBlock->blockId;
           edgeFrom < consumerBlock->blockId; ++edgeFrom) {
        unsigned edgeTo = edgeFrom + 1;
        auto key = std::make_tuple(value, edgeFrom, edgeTo);
        Instance *fifo = pipelineEdgeStorage.lookup(key);
        if (!fifo) {
          unsigned counter = valueRegCounterPerProducer[edgeFrom]++;
          ...
          std::string fifoName = namePrefix + "b" +
                                 std::to_string(edgeFrom) + "b" +
                                 std::to_string(edgeTo) + "v" +
                                 std::to_string(counter);
          fifo = mainModule->addInstance(
              fifoName, fifoMod, {mainClk.getValue(), mainRst.getValue()});
          pipelineEdgeStorage[key] = fifo;
        }

        blocks[edgeFrom].output_fifos[value].push_back(
            std::make_pair(&blocks[edgeTo], fifo));
        blocks[edgeTo].input_fifos[value] = fifo;
        blocks[edgeFrom].pipeline_live_out_fifos[value].push_back(
            std::make_pair(&blocks[edgeTo], fifo));
        blocks[edgeTo].pipeline_live_in_fifos[value] = fifo;
        lastFIFO = fifo;
      }

      flow.storage = lastFIFO;
      continue;
    }
...
LogicalResult BlockHandler::processAllBlocks() {
  ...
  for (unsigned i = 0; i < blocks.size(); i++) {
    ...
    llvm::DenseMap<Value, Instance *> preservedPipelineInputs;
    if (pipelineMode)
      preservedPipelineInputs = block.input_fifos;
    block.input_fifos.clear();
    if (pipelineMode)
      block.input_fifos = preservedPipelineInputs;
    if (pipelineMode) {
      for (auto &[value, fifo] : block.pipeline_live_in_fifos)
        if (fifo)
          block.input_fifos[value] = fifo;
    }
    for (auto &[inputValue, inputFIFO] : input_fifos) {
    ...
    if (pipelineMode) {
      for (CrossBlockValueFlow &flow : crossBlockFlows) {
        if (flow.consumer_block == &block && flow.storage &&
            !block.input_fifos.count(flow.value))
    ...
    if (i == blocks.size() - 1) {
      ...
      for (auto &[value, consumers] : output_fifos) {
        auto &dst = block.output_fifos[value];
        for (auto &consumer : consumers) {
          bool exists = false;
          for (auto &existing : dst) {
            if (existing.second == consumer.second) {
              exists = true;
              break;
            }
          }
          if (!exists)
            dst.push_back(consumer);
        }
      }
    }
    if (pipelineMode) {
      for (auto &[value, consumers] : block.pipeline_live_out_fifos) {
        auto &dst = block.output_fifos[value];
        for (auto &consumer : consumers) {
          bool exists = false;
          for (auto &existing : dst) {
    ...
    if (pipelineMode) {
      for (CrossBlockValueFlow &flow : crossBlockFlows) {
        if (flow.producer_block == &block && flow.storage &&
            !block.output_fifos.count(flow.value))
  ...
  for (BlockInfo &block : blocks) {
    if (failed(processBlock(block))) {
      return funcOp.emitError()
             << "APSToCMT2 failed while processing block " << block.blockId
             << " (" << block.blockName << ")";
    }
  }
