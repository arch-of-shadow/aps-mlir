...
static bool isValidIndex(mlir::Value val);
  ...
  if (recur) {
    if (isa<SelectOp, IndexCastOp, AddIOp, MulIOp, DivSIOp, DivUIOp, RemSIOp,
      ...
      if (llvm::all_of(defOp->getOperands(), [&](Value v) {
            return isValidSymbolInt(v, recur);
          }))
        return true;
  }
  return false;
...
// Helper function from polygeist - checks if value is an affine for loop induction variable
// Updated MLIR API: use llvm::isa/cast instead of val.isa/cast
static bool isAffineForArg(Value val) {
  // Updated API: use llvm::isa instead of val.isa
  if (!llvm::isa<BlockArgument>(val))
    return false;
  // Updated API: use llvm::cast instead of val.cast
  Operation *parentOp = llvm::cast<BlockArgument>(val).getOwner()->getParentOp();
  return (
      isa_and_nonnull<affine::AffineForOp, affine::AffineParallelOp>(parentOp));
}

// Helper function from polygeist - checks if operand needs further composition
// Determines if an operand can be composed into an affine expression
static bool legalCondition(Value en, bool dim = false) {
  ...
  if (!dim) {
    // Updated API: use llvm::dyn_cast instead of en.dyn_cast
...
// Full implementation from polygeist's AffineCFG.cpp, adapted for current MLIR API
bool isValidIndex(mlir::Value val) {
  ...
  // Updated API: use llvm::dyn_cast instead of val.dyn_cast
...
// Full implementation from polygeist's Ops.cpp (lines 553-581)
// Checks if an operation is read-only (only has Read memory effects, no writes)
// This is used by AffineApplyNormalizer to check if operations can be safely moved
...
// Helper function from polygeist - checks if an affine map needs further composition
// Returns true if any operands can be further composed into the affine expression
...
/// The AffineApplyNormalizer composes AffineApplyOp recursively. Its purpose is to
...
// Implementation of AffineApplyNormalizer constructor
// This is the core of the affine map composition logic from polygeist
// Updated for current MLIR API (llvm::isa/cast/dyn_cast, ConstantOp changes)
AffineApplyNormalizer::AffineApplyNormalizer(AffineMap map,
  ...
  [[maybe_unused]] unsigned numSymbols = map.getNumSymbols();
  ...
  std::function<Value(Value, bool)> fix = [&](Value v,
    ...
    if (!op)
      llvm::errs() << v << "\n";
    assert(op);
    // Updated API: use isa instead of dyn_cast for type checking
    ...
    for (auto o : ops) {
      ...
      if (auto *op = o.getDefiningOp()) {
        ...
        // Updated API: use llvm::cast instead of o.cast
        auto BA = llvm::cast<BlockArgument>(o);
...
AffineDimExpr AffineApplyNormalizer::renumberOneDim(Value v) {
  ...
  // Updated API: use dyn_cast on the result instead of cast() method
  return llvm::dyn_cast<AffineDimExpr>(
      getAffineDimExpr(iterPos->second, v.getContext()));
}

// Compose affine map with its operands using AffineApplyNormalizer
// This is a helper called by fully2ComposeAffineMapAndOperands
...
// Full polygeist implementation from AffineCFG.cpp - now with all helper functions
// Recursively composes affine maps and operands until no further composition is needed
// Uses AffineApplyNormalizer for complex affine map composition with dominance analysis
void fully2ComposeAffineMapAndOperands(
    mlir::PatternRewriter &builder, mlir::AffineMap *map,
    llvm::SmallVectorImpl<mlir::Value> *operands, mlir::DominanceInfo &DI) {
  IRMapping indexMap;

  // Build a map of index casts that can be reused
  ...
  // Full polygeist implementation: iteratively compose until no operands need composition
  ...
  *map = simplifyAffineMap(*map);
  ...
  // Ensure all operands are index type
  for (auto &op : *operands) {
    if (!op.getType().isIndex()) {
      ...
      if (auto *o = op.getDefiningOp())
        ...
        // Updated MLIR API: use llvm::cast instead of op.cast
        auto BA = llvm::cast<BlockArgument>(op);
