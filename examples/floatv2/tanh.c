...
static inline uint32_t tanh_custom(uint32_t addr_in, uint32_t addr_out) {
    uint32_t rd = 0;
    asm volatile(".insn r 0x0B, 0b111, 0x31, %0, %1, %2" : "=r"(rd) : "r"(addr_in), "r"(addr_out));
    asm volatile("fence");
    return rd;
}
...
int main(void) {
    ...
    printf("Calling Tanh accelerator...\n");
    (void)tanh_custom((uint32_t)(uintptr_t)input_bits, (uint32_t)(uintptr_t)output_bits);
