/**
 * Tanh - Hyperbolic Tangent (Approximate Version)
 * Formula: tanh(x) ≈ 2 * sigmoid(2x) - 1
 */
...
static inline uint32_t tanh_approx_custom(uint32_t addr_in, uint32_t addr_out) {
    uint32_t rd = 0;
    asm volatile(".insn r 0x0B, 0b111, 0x32, %0, %1, %2" : "=r"(rd) : "r"(addr_in), "r"(addr_out));
    asm volatile("fence");
    return rd;
}
...
static float tanh_approx_cpu(float x) { return 2.0f * sigmoid(2.0f * x) - 1.0f; }
...
int main(void) {
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
        ...
        expected[i] = tanh_approx_cpu(input[i]);
    }

    print_matrix("Expected (CPU)", expected, ROWS, COLS);

    printf("Calling Tanh Approx accelerator...\n");
    (void)tanh_approx_custom((uint32_t)(uintptr_t)input_bits, (uint32_t)(uintptr_t)output_bits);
