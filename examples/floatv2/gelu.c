...
static inline uint32_t gelu_custom(uint32_t addr_in, uint32_t addr_out) {
    uint32_t rd = 0;
    asm volatile(".insn r 0x0B, 0b111, 0x2D, %0, %1, %2" : "=r"(rd) : "r"(addr_in), "r"(addr_out));
    asm volatile("fence");
    return rd;
}
...
int main(void) {
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
        ...
        expected[i] = gelu_cpu(input[i]);
    }

    print_matrix("Expected (CPU)", expected, ROWS, COLS);

    printf("Calling GELU accelerator...\n");
    (void)gelu_custom((uint32_t)(uintptr_t)input_bits, (uint32_t)(uintptr_t)output_bits);
