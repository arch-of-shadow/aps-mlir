...
static inline uint32_t rmsnorm_custom(uint32_t addr_in, uint32_t addr_out) {
    uint32_t rd = 0;
    asm volatile(".insn r 0x0B, 0b111, 0x30, %0, %1, %2" : "=r"(rd) : "r"(addr_in), "r"(addr_out));
    asm volatile("fence");
    return rd;
}
...
static void rmsnorm_row_cpu(const float* in, float* out, int n) {
    float sum_sq = 0;
    for (int i = 0; i < n; i++) sum_sq += in[i] * in[i];
    float rms = sqrtf(sum_sq / n + EPSILON);
    for (int i = 0; i < n; i++) out[i] = in[i] / rms;
}
...
int main(void) {
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
        input[i] = random_float(-2.0f, 2.0f);
        input_bits[i] = float_to_bits(input[i]);
    }

    for (int r = 0; r < ROWS; r++)
        rmsnorm_row_cpu(&input[r * COLS], &expected[r * COLS], COLS);

    print_matrix("Expected (CPU)", expected, ROWS, COLS);

    printf("Calling RMSNorm accelerator...\n");
    (void)rmsnorm_custom((uint32_t)(uintptr_t)input_bits, (uint32_t)(uintptr_t)output_bits);
