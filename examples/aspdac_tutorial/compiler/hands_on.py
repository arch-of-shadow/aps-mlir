...
def render_step2():
    ...
    if snapshots:
        ...
        # Show E-graph stats for key phases
        ...
        with col1:
            st.metric("Initial", f"{init_ec} / {init_en}", help="E-classes / E-nodes")
        with col2:
            delta_ec = internal_ec - init_ec
            st.metric("After Internal", f"{internal_ec} / {internal_en}",
                     delta=f"+{delta_ec}" if delta_ec > 0 else str(delta_ec))
        with col3:
            delta_ec = external_ec - internal_ec
            st.metric("After External", f"{external_ec} / {external_en}",
                     delta=f"+{delta_ec}" if delta_ec > 0 else str(delta_ec))
        with col4:
            delta_ec = custom_ec - external_ec
            st.metric("After Custom", f"{custom_ec} / {custom_en}",
                     delta=f"+{delta_ec}" if delta_ec > 0 else str(delta_ec))

        # ============ Phase Timeline (up to custom rewrites) ============
        st.markdown("---")
        st.markdown("### Phase Timeline")

        timeline_data = []
        for s in snapshots:
            # Only include phases up to custom rewrites
            if '3.5' in s['phase_name'] or '4_' in s['phase_name']:
                continue

            egraph_stats = s.get('egraph_stats', {})
            eclasses = 0
            enodes = 0
            for stats in egraph_stats.values():
                if isinstance(stats, dict):
                    eclasses = stats.get('total_eclasses', 0)
                    enodes = stats.get('total_nodes', 0)
                    break

            cum = s.get('cumulative_stats', {})
            timeline_data.append({
                "Phase": s['phase_name'],
                "E-Classes": eclasses,
                "E-Nodes": enodes,
                "Internal": cum.get("internal_rewrites", 0),
                "External": cum.get("external_rewrites", 0),
                "Custom": cum.get("custom_rewrites", 0),
            })
        ...
        # ============ External Loop Transformations ============
        st.markdown("---")
        st.markdown("### External Loop Transformations")

        skeleton_info = st.session_state.get("skeleton_info", {})
        render_external_loop_transforms(skeleton_info)

        # ============ Skeleton & Components (from compile output) ============
        st.markdown("---")
        st.markdown("### Skeleton & Components")

        # Get skeleton info from compile output
        skeleton_info = st.session_state.get("skeleton_info", {})

        # Display skeleton as control flow graph with components inline
        render_skeleton_and_components(skeleton_info)

        # ============ Custom Instruction Matching ============
        st.markdown("---")
        st.markdown("### Custom Matching Result")

        # Get custom rewrites count
        ...
        # ============ E-graph Visualization (Expandable) ============
        st.markdown("---")
        st.markdown("### E-graph Visualization")

        # Check for SVG files in temp directory
        from megg.utils import get_temp_dir
        tmp_dir = get_temp_dir()
        svg_files = {
            "Initial": tmp_dir / "egraph_before_internal.svg",
            "After Internal": tmp_dir / "egraph_after_internal.svg",
            "After Custom": tmp_dir / "egraph_after_custom.svg",
        }

        for label, svg_path in svg_files.items():
            if svg_path.exists():
                with st.expander(f"E-graph: {label}", expanded=False):
                    try:
                        svg_content = svg_path.read_text()
                        st.markdown(f'<div style="overflow: auto; max-height: 400px;">{svg_content}</div>',
                                   unsafe_allow_html=True)
                    except Exception as e:
                        st.warning(f"Could not load SVG: {e}")
            else:
                with st.expander(f"E-graph: {label}", expanded=False):
                    st.info("SVG not available. Run with visualization enabled to generate.")

    elif not st.session_state.get("matching_complete", False):
        # Not run yet
        st.markdown("---")
        st.markdown("### E-graph Evolution")
        st.info("Run E-graph Matching to see phase statistics")

        st.markdown("---")
        st.markdown("### Phase Timeline")
        st.info("Run E-graph Matching to see phase timeline")

        st.markdown("---")
        st.markdown("### Custom Instruction Matching")
        st.info("Run E-graph Matching to see matching results")

    # ============ Assembly Output ============
    st.markdown("---")
    st.markdown("### Assembly Output")
