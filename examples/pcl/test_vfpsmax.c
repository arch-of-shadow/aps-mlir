...
uint32_t vfpsmax_v(uint32_t rs1, uint32_t rs2) {
  uint32_t rd = 0;
  asm volatile(
  ".insn r 0x2B, 0b111, 0x2A, %0, %1, %2"  // opcode=0x2B (0101011), funct7=0x2A (0101010)
  : "=r"(rd) : "r"(rs1), "r"(rs2));
  return rd;
}

// Input data: 16 distance values (u32)
// This is used in Farthest Point Sampling to find the point with maximum distance
...
int main(void) {
  ...
  uint32_t result = vfpsmax_v((uint32_t)(unsigned long)input_data,
  ...
  printf("\nValue match: %s (expected=%u, got=%u)\n",
         value_match ? "✓" : "✗", expected_max, output_data[0]);
  printf("Index match: %s (expected=%u, got=%u)\n",
         index_match ? "✓" : "✗", expected_idx, output_data[1]);
  printf("RD match: %s (expected=%u, got=%u)\n",
         rd_match ? "✓" : "✗", expected_idx, result);
