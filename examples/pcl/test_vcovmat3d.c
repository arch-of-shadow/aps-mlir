...
uint32_t vcovmat3d_vv(uint32_t rs1, uint32_t rs2) {
  uint32_t rd = 0;
  asm volatile(
  ".insn r 0x2B, 0b111, 0x30, %0, %1, %2"  // opcode=0x2B (0101011), funct7=0x30 (0110000)
  : "=r"(rd) : "r"(rs1), "r"(rs2));
  return rd;
}
...
int main(void) {
  ...
  uint32_t result = vcovmat3d_vv((uint32_t)(unsigned long)input_data,
  ...
  printf("  [%10d %10d %10d]\n", output_data[1], output_data[3], output_data[4]);
  printf("  [%10d %10d %10d]\n", output_data[2], output_data[4], output_data[5]);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    printf("%s: expected=%d, got=%d %s\n",
           names[i], expected[i], output_data[i],
           match ? "✓" : "✗");
