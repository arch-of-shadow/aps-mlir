...
// Input data: 16 3D points in Structure-of-Arrays layout + 1 reference point
// Memory layout:
// - points_x[16] at offset 0
// - points_y[16] at offset 64 (16*4 bytes)
...
int main(void) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    uint32_t ref_y = input_data[49];
    uint32_t ref_z = input_data[50];
