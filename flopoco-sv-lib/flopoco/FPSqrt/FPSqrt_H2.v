/* Generated by Yosys 0.53 (git sha1 53c22ab7c, ccache clang++ 18.1.3 -fPIC -O3) */

(* top =  1  *)
/* verilator lint_off CASEOVERLAP*/
module FPSqrt_H2(clk, rst, X, R);
  wire [4:0] _00_;
  wire [5:0] _01_;
  wire _02_;
  wire _03_;
  wire [6:0] _04_;
  wire [6:0] _05_;
  wire [6:0] _06_;
  wire _07_;
  wire _08_;
  wire [7:0] _09_;
  wire [7:0] _10_;
  wire [4:0] _11_;
  wire [7:0] _12_;
  wire _13_;
  wire _14_;
  wire [8:0] _15_;
  wire [8:0] _16_;
  wire [8:0] _17_;
  wire _18_;
  wire _19_;
  wire [9:0] _20_;
  wire [9:0] _21_;
  wire _22_;
  wire [9:0] _23_;
  wire _24_;
  wire _25_;
  wire [10:0] _26_;
  wire [10:0] _27_;
  wire [10:0] _28_;
  wire _29_;
  wire _30_;
  wire [11:0] _31_;
  wire [11:0] _32_;
  wire [13:0] _33_;
  wire [11:0] _34_;
  wire _35_;
  wire _36_;
  wire [12:0] _37_;
  wire [12:0] _38_;
  wire [12:0] _39_;
  wire _40_;
  wire _41_;
  wire [13:0] _42_;
  wire [13:0] _43_;
  wire [3:0] _44_;
  wire [13:0] _45_;
  wire _46_;
  wire _47_;
  wire [14:0] _48_;
  wire [14:0] _49_;
  wire [14:0] _50_;
  wire _51_;
  wire [9:0] _52_;
  wire _53_;
  wire _54_;
  wire _55_;
  wire _56_;
  wire _57_;
  wire [2:0] _58_;
  reg [4:0] _59_;
  reg [4:0] _60_;
  reg _61_;
  reg [8:0] _62_;
  reg [5:0] _63_;
  reg [8:0] _64_;
  reg [5:0] _65_;
  wire _66_;
  reg _67_;
  reg [13:0] _68_;
  reg _69_;
  reg [13:0] _70_;
  reg [10:0] _71_;
  reg [2:0] _72_;
  reg [2:0] _73_;
  wire [5:0] _74_;
  wire [5:0] _75_;
  output [17:0] R;
  wire [17:0] R;
  input [17:0] X;
  wire [17:0] X;
  input clk;
  wire clk;
  wire d1;
  wire d10;
  wire d12;
  wire d2;
  wire d3;
  wire d4;
  wire d4_d1;
  wire d5;
  wire d6;
  wire d7;
  wire d8;
  wire d9;
  wire d9_d1;
  wire [4:0] ern0;
  wire [4:0] ern1;
  wire [4:0] ern1_d1;
  wire [4:0] ern1_d2;
  wire [9:0] fr;
  wire [9:0] fracx;
  wire [13:0] fracxnorm;
  wire [9:0] frrnd;
  wire [12:0] mr;
  wire [14:0] rn2;
  wire round;
  input rst;
  wire rst;
  wire [1:0] s0;
  wire [2:0] s1;
  wire [11:0] s10;
  wire [3:0] s2;
  wire [4:0] s3;
  wire [5:0] s4;
  wire [5:0] s4_d1;
  wire [6:0] s5;
  wire [7:0] s6;
  wire [8:0] s7;
  wire [9:0] s8;
  wire [10:0] s9;
  wire [10:0] s9_d1;
  wire [13:0] t1;
  wire [13:0] t10;
  wire [12:0] t10_h;
  wire [14:0] t10s;
  wire [14:0] t10s_h;
  wire [13:0] t11;
  wire [13:0] t11_h;
  wire [14:0] t12_h;
  wire [14:0] t1s;
  wire [5:0] t1s_h;
  wire [8:0] t1s_l;
  wire [13:0] t2;
  wire [14:0] t2s;
  wire [6:0] t2s_h;
  wire [7:0] t2s_l;
  wire [13:0] t3;
  wire [5:0] t3_h;
  wire [14:0] t3s;
  wire [7:0] t3s_h;
  wire [6:0] t3s_l;
  wire [13:0] t4;
  wire [6:0] t4_h;
  wire [14:0] t4s;
  wire [8:0] t4s_h;
  wire [8:0] t4s_h_d1;
  wire [5:0] t4s_l;
  wire [5:0] t4s_l_d1;
  wire [13:0] t5;
  wire [7:0] t5_h;
  wire [14:0] t5s;
  wire [9:0] t5s_h;
  wire [4:0] t5s_l;
  wire [13:0] t6;
  wire [8:0] t6_h;
  wire [14:0] t6s;
  wire [10:0] t6s_h;
  wire [3:0] t6s_l;
  wire [13:0] t7;
  wire [9:0] t7_h;
  wire [14:0] t7s;
  wire [11:0] t7s_h;
  wire [2:0] t7s_l;
  wire [13:0] t8;
  wire [10:0] t8_h;
  wire [14:0] t8s;
  wire [12:0] t8s_h;
  wire [1:0] t8s_l;
  wire [13:0] t9;
  wire [11:0] t9_h;
  wire [14:0] t9s;
  wire [13:0] t9s_h;
  wire [13:0] t9s_h_d1;
  wire t9s_l;
  wire t9s_l_d1;
  wire [5:0] u1;
  wire [14:0] u10;
  wire [6:0] u2;
  wire [7:0] u3;
  wire [8:0] u4;
  wire [8:0] u4_d1;
  wire [9:0] u5;
  wire [10:0] u6;
  wire [11:0] u7;
  wire [12:0] u8;
  wire [13:0] u9;
  wire [13:0] u9_d1;
  wire [2:0] xsr;
  wire [2:0] xsr_d1;
  wire [2:0] xsr_d2;
  wire [2:0] xsx;
  assign _03_ = ~ d2;
  assign _04_ = t2s_h - u2;
  assign _05_ = d2 ? _04_ : _06_;
  assign _06_ = t2s_h + u2;
  assign _07_ = ~ t3[13];
  assign _08_ = ~ d3;
  assign _09_ = t3s_h - u3;
  assign _10_ = d3 ? _09_ : _12_;
  assign _12_ = t3s_h + u3;
  assign _13_ = ~ t4[13];
  assign _14_ = ~ d4;
  assign _15_ = t4s_h_d1 - u4_d1;
  assign _16_ = d4_d1 ? _15_ : _17_;
  assign _17_ = t4s_h_d1 + u4_d1;
  assign _18_ = ~ t5[13];
  assign _19_ = ~ d5;
  assign _20_ = t5s_h - u5;
  assign _21_ = d5 ? _20_ : _23_;
  assign _23_ = t5s_h + u5;
  assign _24_ = ~ t6[13];
  assign _25_ = ~ d6;
  assign _26_ = t6s_h - u6;
  assign _27_ = d6 ? _26_ : _28_;
  assign _28_ = t6s_h + u6;
  assign _29_ = ~ t7[13];
  assign _30_ = ~ d7;
  assign _31_ = t7s_h - u7;
  assign _32_ = d7 ? _31_ : _34_;
  assign _34_ = t7s_h + u7;
  assign _35_ = ~ t8[13];
  assign _36_ = ~ d8;
  assign _37_ = t8s_h - u8;
  assign _38_ = d8 ? _37_ : _39_;
  assign _39_ = t8s_h + u8;
  assign _40_ = ~ t9[13];
  assign _41_ = ~ d9;
  assign _42_ = t9s_h_d1 - u9_d1;
  assign _43_ = d9_d1 ? _42_ : _45_;
  assign _45_ = t9s_h_d1 + u9_d1;
  assign _46_ = ~ t10[13];
  assign _47_ = ~ d10;
  assign _48_ = t10s_h - u10;
  assign _49_ = d10 ? _48_ : _50_;
  assign _50_ = t10s_h + u10;
  assign _51_ = ~ t11[13];
  assign _52_ = fr + { 9'h000, round };
  assign _53_ = xsx == 3'h2;
  assign _54_ = xsx == 3'h4;
  assign _56_ = xsx == 3'h0;
  assign _57_ = xsx == 3'h1;
  function [2:0] \:284 ;
    input [2:0] a;
    input [11:0] b;
    input [3:0] s;
    (* parallel_case *)
    casez (s)
      4'b???1:
        \:284  = b[2:0];
      4'b??1?:
        \:284  = b[5:3];
      4'b?1??:
        \:284  = b[8:6];
      4'b1???:
        \:284  = b[11:9];
      default:
        \:284  = a;
    endcase
  endfunction
  assign _58_ = \:284 (3'h6, 12'h222, { _57_, _56_, _54_, _53_ });
  always @(posedge clk, posedge rst)
    if (rst) _59_ <= 5'h00;
    else _59_ <= ern1;
  always @(posedge clk, posedge rst)
    if (rst) _60_ <= 5'h00;
    else _60_ <= ern1_d1;
  always @(posedge clk, posedge rst)
    if (rst) _61_ <= 1'h0;
    else _61_ <= d4;
  always @(posedge clk, posedge rst)
    if (rst) _62_ <= 9'h000;
    else _62_ <= t4s_h;
  always @(posedge clk, posedge rst)
    if (rst) _63_ <= 6'h00;
    else _63_ <= t4s_l;
  always @(posedge clk, posedge rst)
    if (rst) _64_ <= 9'h000;
    else _64_ <= u4;
  always @(posedge clk, posedge rst)
    if (rst) _65_ <= 6'h00;
    else _65_ <= s4;
  always @(posedge clk, posedge rst)
    if (rst) _67_ <= 1'h0;
    else _67_ <= d9;
  always @(posedge clk, posedge rst)
    if (rst) _68_ <= 14'h0000;
    else _68_ <= t9s_h;
  always @(posedge clk, posedge rst)
    if (rst) _69_ <= 1'h0;
    else _69_ <= t9s_l;
  always @(posedge clk, posedge rst)
    if (rst) _70_ <= 14'h0000;
    else _70_ <= u9;
  always @(posedge clk, posedge rst)
    if (rst) _71_ <= 11'h000;
    else _71_ <= s9;
  always @(posedge clk, posedge rst)
    if (rst) _72_ <= 3'h0;
    else _72_ <= xsr;
  always @(posedge clk, posedge rst)
    if (rst) _73_ <= 3'h0;
    else _73_ <= xsr_d1;
  assign _00_ = ern0 + 5'h07;
  assign _11_ = _00_ + { 4'h0, X[10] };
  assign _22_ = ~ X[10];
  assign _33_ = _22_ ? { 1'h1, fracx, 3'h0 } : { 2'h1, fracx, 2'h0 };
  assign _44_ = 4'h7 + fracxnorm[13:10];
  assign _55_ = ~ t1[13];
  assign _66_ = ~ d1;
  assign _74_ = t1s_h - u1;
  assign _75_ = d1 ? _74_ : _01_;
  assign _01_ = t1s_h + u1;
  assign _02_ = ~ t2[13];
  assign fracx = X[9:0];
  assign ern0 = { 1'h0, X[14:11] };
  assign xsx = X[17:15];
  assign ern1 = _11_;
  assign ern1_d1 = _59_;
  assign ern1_d2 = _60_;
  assign fracxnorm = _33_;
  assign s0 = 2'h1;
  assign t1 = { _44_, fracxnorm[9:0] };
  assign d1 = _55_;
  assign t1s = { t1, 1'h0 };
  assign t1s_h = t1s[14:9];
  assign t1s_l = t1s[8:0];
  assign u1 = { 1'h0, s0, d1, _66_, 1'h1 };
  assign t3_h = _75_;
  assign t2 = { t3_h[4:0], t1s_l };
  assign s1 = { s0, d1 };
  assign d2 = _02_;
  assign t2s = { t2, 1'h0 };
  assign t2s_h = t2s[14:8];
  assign t2s_l = t2s[7:0];
  assign u2 = { 1'h0, s1, d2, _03_, 1'h1 };
  assign t4_h = _05_;
  assign t3 = { t4_h[5:0], t2s_l };
  assign s2 = { s1, d2 };
  assign d3 = _07_;
  assign t3s = { t3, 1'h0 };
  assign t3s_h = t3s[14:7];
  assign t3s_l = t3s[6:0];
  assign u3 = { 1'h0, s2, d3, _08_, 1'h1 };
  assign t5_h = _10_;
  assign t4 = { t5_h[6:0], t3s_l };
  assign s3 = { s2, d3 };
  assign d4 = _13_;
  assign d4_d1 = _61_;
  assign t4s = { t4, 1'h0 };
  assign t4s_h = t4s[14:6];
  assign t4s_h_d1 = _62_;
  assign t4s_l = t4s[5:0];
  assign t4s_l_d1 = _63_;
  assign u4 = { 1'h0, s3, d4, _14_, 1'h1 };
  assign u4_d1 = _64_;
  assign t6_h = _16_;
  assign t5 = { t6_h[7:0], t4s_l_d1 };
  assign s4 = { s3, d4 };
  assign s4_d1 = _65_;
  assign d5 = _18_;
  assign t5s = { t5, 1'h0 };
  assign t5s_h = t5s[14:5];
  assign t5s_l = t5s[4:0];
  assign u5 = { 1'h0, s4_d1, d5, _19_, 1'h1 };
  assign t7_h = _21_;
  assign t6 = { t7_h[8:0], t5s_l };
  assign s5 = { s4_d1, d5 };
  assign d6 = _24_;
  assign t6s = { t6, 1'h0 };
  assign t6s_h = t6s[14:4];
  assign t6s_l = t6s[3:0];
  assign u6 = { 1'h0, s5, d6, _25_, 1'h1 };
  assign t8_h = _27_;
  assign t7 = { t8_h[9:0], t6s_l };
  assign s6 = { s5, d6 };
  assign d7 = _29_;
  assign t7s = { t7, 1'h0 };
  assign t7s_h = t7s[14:3];
  assign t7s_l = t7s[2:0];
  assign u7 = { 1'h0, s6, d7, _30_, 1'h1 };
  assign t9_h = _32_;
  assign t8 = { t9_h[10:0], t7s_l };
  assign s7 = { s6, d7 };
  assign d8 = _35_;
  assign t8s = { t8, 1'h0 };
  assign t8s_h = t8s[14:2];
  assign t8s_l = t8s[1:0];
  assign u8 = { 1'h0, s7, d8, _36_, 1'h1 };
  assign t10_h = _38_;
  assign t9 = { t10_h[11:0], t8s_l };
  assign s8 = { s7, d8 };
  assign d9 = _40_;
  assign d9_d1 = _67_;
  assign t9s = { t9, 1'h0 };
  assign t9s_h = t9s[14:1];
  assign t9s_h_d1 = _68_;
  assign t9s_l = t9s[0];
  assign t9s_l_d1 = _69_;
  assign u9 = { 1'h0, s8, d9, _41_, 1'h1 };
  assign u9_d1 = _70_;
  assign t11_h = _43_;
  assign t10 = { t11_h[12:0], t9s_l_d1 };
  assign s9 = { s8, d9 };
  assign s9_d1 = _71_;
  assign d10 = _46_;
  assign t10s = { t10, 1'h0 };
  assign t10s_h = t10s;
  assign u10 = { 1'h0, s9_d1, d10, _47_, 1'h1 };
  assign t12_h = _49_;
  assign t11 = t12_h[13:0];
  assign s10 = { s9_d1, d10 };
  assign d12 = _51_;
  assign mr = { s10, d12 };
  assign fr = mr[10:1];
  assign round = mr[0];
  assign frrnd = _52_;
  assign rn2 = { ern1_d2, frrnd };
  assign xsr = _58_;
  assign xsr_d1 = _72_;
  assign xsr_d2 = _73_;
  assign R = { xsr_d2, rn2 };
endmodule
/* verilator lint_on CASEOVERLAP*/
