...
public:
  ...
  SDCOpWrapper(OpAbstract *op) : OpWrapperBase(op, OK_SDCWRAPPER) {}

  int getStartTime() override { return SDCTime; }
...
private:
  int get_memportMII(Loop *L);
  int getMAxiMII(Loop *L);
  int resourceMII(Loop *L);

  int recurrenceMII(Loop *L);

  // after pipeline schedule achieved, check if II is greater than or equal to loop cycle
  bool isScheduleResultWithIIValid(Loop *L, int II);
   ...
   * @param FinalFlag Whether optimize register lifetime if schedule with II can
   * be achieved.
   */
   ...
   */
  void traverse(SDCOpWrapper *op, SDCSolver *SDC, int latency, float cp,
                int dist, int II, SDCOpWrapper *start,
   ...
   * Currently, implement a exponential algorithm which enumerate all possible
   * simple paths.
   */
  ...
  /**
   * Get a reference solution without resource constraint as Described in the
   * paper This solution minimize the register pressure. Due to extra
   * constraints added, SDC solver can not solve this, a lp solver is needed.
   */
  ...
  /**
   * Find critical paths starting from op. Only dependencies with distance 0 are
   * considered. Using SPFA.
  ...
  addResourceConstrBB(BasicBlock *BB, std::vector<std::vector<int>> &&Vars,
                      int RId, SDCSolver *SDC);
  ...
  getFeasibleOrder(BasicBlock *BB, function_ref<bool(SDCOpWrapper *)> Pred);

  /// add memory resource constraint
  void addMAxiConstr(SDCSolver *SDC);
  void addMemConstr(int RId, SDCSolver *SDC);
  void assignSlotAttrAfterSchedule();

  SDCSolver *formulateSDC();

  void FunctionAllocVariable(std::unordered_map<BasicBlock *, int> &BeginBB,std::unordered_map<BasicBlock *, int> &EndBB,SDCSolver *SDC);
  void FunctionformulateDependency(std::unordered_map<BasicBlock *, int> &BeginBB,std::unordered_map<BasicBlock *, int> &EndBB,SDCSolver *SDC);
  bool FunctiongetASAPTime(SDCSolver *SDC);
