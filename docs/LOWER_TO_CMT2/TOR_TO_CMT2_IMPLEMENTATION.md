...
# Test specific file
./circt/build/bin/llvm-lit -v test/Dialect/Cmt2/gcd.mlir

# Run circt-opt
./circt/build/bin/circt-opt <input.mlir> [passes]

# Run firtool (FIRRTL to Verilog)
./circt/build/bin/firtool <input.mlir> --format=mlir -o output.sv
```
...
### Rule Translation Roadmap

This roadmap is intended to be followed verbatim by the compiler pass. Every instruction below describes a concrete transformation the pass must perform on the TOR IR to produce CMT2 hardware.

#### Phase A – Straight-Line Kernel Support (No APS, No Loops)

**Scope**: Subset of `1.mlir` where the accelerator executes straight-line datapaths—no APS interactions or loop control. The goal is to emit CMT2 rules whose guards/bodies directly implement the scheduled datapath, talking to the scratchpad banks through the native ports provided by the memory pool; if the TOR schedule crosses cycle boundaries, insert FIFO instances (from the memory pool pipeline) between producer/consumer rules to preserve timing.

**Deliverables (hardware view)**:
1. Per-slot rule emitter: each time-slot window becomes a `cmt2.rule` with a single-bit ready guard and a body that clones all scheduled operations for that window.
2. Direct scratchpad access: rules must call the bank methods emitted by the memory-pool pass (`*_bank<i>_{read,write}`) so the scratchpad behaviour matches TOR.
3. FIFO bridges: insert FIFOs wherever the schedule crosses slots (see steps below), naming them `fifo_s<producerSlot>_s<consumerSlot>` (append `_<n>` for multiple values).
4. Regression infrastructure: add `test/APS/rule-gen-basic.mlir` plus FileCheck run lines exercising CallInfo, ConflictMatrix, Scheduler, and LowerCmt2ToFIRRTL.

**Steps** (implement each item exactly as written):
1. Traverse the `tor.timegraph` blocks in ascending `starttime` order. For every distinct slot index, collect the TOR/arith/FIRRTL ops whose `[starttime, endtime)` interval covers that slot. Create one `cmt2.rule` per slot. Name the rule `@rule_s<slotIndex>`.
2. Emit the rule’s guard region by AND-ing together required ready bits:
   - If TOR declares an explicit predicate (e.g., a scheduled conditional), reproduce it with FIRRTL ops returning a single `!firrtl.uint<1>`.
   - If no predicate is required, return a constant `1`.
   - (Note: FIFO consumption does not require an explicit guard—the `@dequeue` call inside the body causes the CMT2 compiler to reference the FIFO’s ready signal automatically.)
3. Emit the rule’s body region:
   - Clone every arithmetic/TOR op scheduled in the slot into the body, replacing operands with values produced in earlier rules or FIFO dequeues.
   - Replace `memref` accesses with `ScratchpadMemoryPool` calls: `cmt2.call @pool @mem_<bank>_<op>(...)` using the original addresses/data.
4. Insert FIFOs for cross-slot dependencies:
   - For each SSA value where `consumer.starttime > producer.endtime`, instantiate (or reuse) a FIFO named `fifo_s<producerSlot>_s<consumerSlot>`; append `_1`, `_2`, … if the same producer feeds multiple values to the same consumer slot.
   - Use the ECMT2 library module `@FIFO1_PUSH_w<width>` (depth-fixed to 1, width = bit width of the value). This module already enforces the correct transactional conflicts.
   - In the producer rule body, enqueue the value. In the consumer rule body, issue `@dequeue` and use the returned value (no explicit guard needed).
5. Emit a `cmt2.precedence` attribute mirroring the exact `tor.succ` edges. For every `tor.succ A -> B`, record `["@rule_sA", "@rule_sB"]`.
6. Run the generated IR through `-cmt2-print-call-info`, `-cmt2-print-conflict-matrix`, `-cmt2-print-scheduler`, `--lower-cmt2-to-firrtl`, and `firtool` to verify correctness.

**Status (2025-10-19)**
- `aps-opt --aps-rule-gen` now emits per-slot `cmt2.rule` bodies for straight-line arithmetic (TOR `addi/subi/muli` + `arith.constant`) and bridges cross-slot values with depth-1 FIFOs named `fifo_sX_sY[_N]`.
- Constants are rematerialized inside each rule so no raw MLIR block arguments leak into CMT2; TOR function arguments remain unsupported and must be unused.
- Regression coverage: `tests/test_cmt2gen/basic-target-a.mlir` plus the new `complex-target-a.mlir` exercise multi-slot dependency fan-out and precedence wiring.
- Next objective in Phase A: lower scratchpad reads/writes by calling the memory-pool methods (`*_bank<i>_read`/`write`) generated by `--aps-memory-pool-gen`.

**Schedule takeaways from real kernels**

- `starttime/endtime` slots increment monotonically even when an operation lasts many real cycles. Rely on slot slack for FIFO placement and fall back to `ref_*` only when precise cycle counts are required (e.g., very long bursts).
- FIFO placement is driven by SSA def-use edges: every consumer window whose slot index exceeds the producer’s gets a dedicated FIFO (`fifo_s<producer>_s<consumer>` plus numeric suffixes). Each FIFO is a depth-1 `FIFO1_PUSH` instance sized only by payload width.
- TOR DAG edges come directly from `tor.succ`; always use those edges when building precedence—even if slot IDs skip—so the generated rules obey the exact execution order.
- Burst pipelines mix transactional (`aps.memburst*`) and element-level (`aps.memload/store`) accesses inside the same window. The rule generator must schedule both interface calls and direct scratchpad accesses simultaneously.
- Loop bodies (`tor.for`) often have non-trivial `unroll` factors. The pass must duplicate or sequence rules accordingly while keeping the ORAAT semantics enforced by conflict/precedence metadata.

#### Phase B – APS Interface Operations & Memory Burst Pipeline

**Scope**: Reintroduce APS datapaths—burst engines, memload/memstore, register-file interactions—while keeping loop structure expanded. The generated rules must speak to APS blocks via ECMT2 interfaces so the handshake wiring remains sound.

**Deliverables (hardware view)**:
1. Interface declarations/definitions for APS ports so the scratchpad and host interact via transactional call semantics (`ready`/`enable`).
2. Rule emission pass that lowers each APS TOR op to an interface method call, wiring arguments/results to the appropriate handshake bundles.
3. FIFO logic inherited from Phase A must still run—multiple APS ops may demand additional FIFOs.
4. Reserve interface definitions for CPU-facing command/response channels (bidirectional) and for host memory (HellaCache) traffic, even if translation logic is added later.
5. Regression: `test/APS/rule-gen-aps.mlir` capturing burst traffic and register file moves, with checks that conflict matrices encode the interface arbitration correctly.

**Steps**:
1. Define transactional interfaces (`InterfaceDecl`, `InterfaceDef`) for burst controllers, register files, etc., binding them to the FIRRTL modules produced by the memory-pool pass.
2. Declare two CPU-facing interfaces (modeled after the RoCC command/response bundles):
   - `@cpu_cmd_if` (host → accelerator): method `@cmd` with payload `[funct:7, rs1:5, rs2:5, rd:5, xs1:1, xs2:1, xd:1, opcode:7, rs1data:32, rs2data:32]`. The interface uses the standard ready/valid handshake with the host driving valid and the accelerator driving ready.
   - `@cpu_resp_if` (accelerator → host): method `@resp` with payload `[rd:5, rddata:32]`. The accelerator drives valid; the host drives ready.
3. Declare host-memory interfaces (HellaCache semantics) separate from scratchpad access:
   - `@cpu_mem_req_if` (accelerator → CPU memory controller): method `@req` with payload `[addr:32, tag:8, cmd:5 (0=read,1=write), size:2, signed:1, phys:1, data:32, mask:4]`. Accelerator drives valid; memory drives ready.
   - `@cpu_mem_resp_if` (CPU memory controller → accelerator): method `@resp` with payload `[addr:32, tag:8, cmd:5, size:2, signed:1, data:32, mask:4]`. Memory drives valid; accelerator drives ready.
   These inserts reserve the transactional endpoints even if the translation logic is implemented later.
4. Extend the rule generator so every APS TOR op emits the appropriate interface method call, maintaining the ready/enable discipline.
5. Apply the same FIFO analysis used in Phase A to APS-produced values; e.g., burst-load results or register reads that cross slots must enqueue into `FIFO1_PUSH` instances with the same naming convention.
6. Translate TOR dependency metadata onto the interfaces: tag methods with conflict/sequential-before relations derived from the APS schedule.
7. Validate CallInfo/Scheduler results to ensure interface arcs appear exactly where the hardware expects them.

#### Phase C – Loop and Control Structure Handling

**Scope**: Full accelerator behaviour: nested `tor.for` loops, overlapping burst pipelines, and host register-file traffic. The generated hardware must sequence iterations while respecting ORAAT semantics and TOR’s schedule.

**Deliverables (hardware view)**:
1. Loop control strategy that either replicates unrolled iterations into distinct rules or sequences them with explicit precedence, matching TOR’s `tor.for` timing.
2. Proper handling of loop-carried storage: introduce wires/registers for values spanning cycles, update guards/bodies to drive them.
3. Interfaces for CPU-visible operations (`aps.readrf`, `aps.writerf`) so the generated accelerator adheres to the same ready/enable contracts as the TOR design.
4. Ensure CPU command/response interfaces and CPU memory interfaces declared in Phase B are connected where loop logic interacts with the host or off-chip memory.
5. Regression: `test/APS/rule-gen-full.mlir` (full `1.mlir` lowering) validated with CallInfo, ConflictMatrix, Scheduler, LowerCmt2ToFIRRTL, and firtool.

**Steps**:
1. Enhance the schedule walker to identify loop bodies and unroll factors; replicate rules for each unrolled lane (naming them `@rule_s<slot>_lane<i>`) or sequence iterations dynamically using additional FIFOs if replication is not viable.
2. For each loop-carried SSA value, instantiate a wire/register (via the wire-default helper module) to hold the value between iterations. Ensure guards/bodies reference those storage elements explicitly.
3. Precedence metadata must include the loop-carried ordering: for every loop successor edge, add `["@rule_sX", "@rule_sY"]` entries, including lane variants when rules were replicated.
4. Apply the same FIFO analysis to loop-carried values that span non-adjacent slots.
5. Model register-file communication with dedicated interfaces, ensuring host transactions respect ready/enable behaviour.
6. Re-run the CMT2 verification pipeline to confirm the accelerator hardware maintains the original TOR execution semantics.

Each phase should conclude with documentation updates (this file plus `IMPLEMENTATION_STATUS.md`), new regression tests, and a passing run of:

```bash
./build/tools/aps-opt/aps-opt test/APS/rule-gen-*.mlir \
  -aps-memory-pool-gen \
  -aps-rule-gen       # new rule generation pass
```

followed by the standard CMT2 verification pipeline (`-cmt2-print-call-info`, `-cmt2-print-conflict-matrix`, `-cmt2-print-scheduler`, `--lower-cmt2-to-firrtl`).
